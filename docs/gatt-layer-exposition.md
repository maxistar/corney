...
- GitHub Actions builds discover the service through this repo's `zephyr/module.yml`, run the
  host/native integration checks, and retain enhanced, stock, legacy-only, minimal-extension,
  peripheral, and settings-reset artifacts. The `corney-left-stock` image keeps the Corney shield,
  keymap, BLE HID behavior, and standard services selected by the pinned ZMK configuration while
  omitting this complete custom service.
...
- The feature can be disabled with `CONFIG_ZMK_GATT_LAYER_EXPOSITION=n`; this is the explicit
  setting used for the stock central artifact.
